...
patterns <- c("0" = "none",   # No pattern for level 0
              "1" = "crosshatch",   # Crosshatch pattern for level 1
              "2" = "stripe",   # Horizontal stripes for level 2
              "3" = "north east lines",  # Diagonal lines for level 3
              "4" = "grid")  # Grid pattern for level 4

# Mapping setup
tm_map_birth_sexedu <- tm_shape(states) +
  tm_polygons("Avg_Birth_Rate", palette=colorRampPalette(c("blue", "pink", "red"))(100),
              title="Average Teen Birth Rate by State") +
  tm_pattern(by = "Levels", pattern.shapes = patterns, pattern.color = "black", pattern.alpha = 0.5,
             pattern.lwd = 0.7, pattern.size = 0.15) +
  tm_borders() +
  tm_layout(
    main.title="Average Teen Birth Rate by State with Sex Education Levels",
    main.title.position="center",
    legend.position="right"
  )

# Print the map
print(tm_map_birth_sexedu)
```
